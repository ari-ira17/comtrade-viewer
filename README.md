...
---
Изменения (текущая версия):
- Окно с информацией о выбранном интервале (левая/правая граница) теперь отображается единожды в правом верхнем углу главного окна, а не рядом с каждым каналом.
- У каналов есть набор дефолтных цветов, назначаемых при загрузке (можно выбрать цвет вручную в выпадающем списке). Рядом с названием канала в списке каналов добавлена цветовая метка (swatch) для наглядности.
- Исправлено поведение привязки цвета: выбор цвета обновляет отображение графика канала через `ChannelPlotViewModel.Color`.
